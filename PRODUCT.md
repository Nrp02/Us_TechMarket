...
- **Nothing on screen is live-ticking.** Intraday snapshots land every 15 minutes, news four times a day, summaries after the close; reads are cached up to 60 seconds. The visitor is looking at a recent state of the world, not a real-time one — the product should never imply otherwise.
...
- **Viewed on laptop and iPad.** Phone is a documented non-target, not an oversight.
...
- **Free tiers only, no billing anywhere.** The binding limit is Gemini at 20 requests per day per project; the app runs at a budget of 8 (4 news cycles + 4 summary batches). Every AI call is batched and scheduled — never per article, per stock page, or per interaction.
...
- **The README is currently a stub.** A professor-facing explanation is still to be written.
...
**WCAG 2.1 AA is the bar**, set by the owner. Concretely, future work must hold: 4.5:1 contrast for body text and 3:1 for large text and UI boundaries, in **both** the default dark theme and the light theme; full keyboard operation of the watchlist picker, symbol switcher and theme toggle with a visible focus indicator; and meaningful semantics for the watchlist table, tab sets and charts.

Two known tensions to carry forward rather than rediscover. Numeric change values already carry an explicit `+`/`−` sign, so they do not depend on colour — but the **sparklines and the intraday chart do**, and need a second channel or an accessible-name equivalent. And the logo plate is deliberately light in both themes because several brand marks carry near-black fills that cannot be recoloured, which constrains contrast work around it.

Phone widths (~390px) are a known unsupported case, documented in `CLAUDE.md`. That is a scope decision about device targets, not an accessibility exemption.
